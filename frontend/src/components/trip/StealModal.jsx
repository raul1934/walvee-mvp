...
import { X, Infinity, Check, Heart } from "lucide-react";
    ...
    >
      ...
      >
        ...
        <div className="mb-6">
          ...
          <p
            id="steal-modal-description"
            className="text-gray-400 text-center text-sm leading-relaxed"
          >
            You're about to copy this trip into your profile. You'll be able to
            edit everything freely — while giving credit to the original
            creator. Each trip derived from another will carry a badge showing
            its origin.
          </p>
        </div>
        ...
        <div className="bg-[#0D0D0D] rounded-xl p-4 mb-6 border border-[#2A2B35]">
          ...
          <div className="flex items-center gap-3 justify-between">
            ...
            <Link
              to={`${createPageUrl(
                "Profile"
              )}/favorites?email=${encodeURIComponent(trip.created_by)}`}
              onClick={(e) => e.stopPropagation()}
              className="inline-flex items-center gap-2 px-3 py-1 rounded-lg bg-black/40 hover:bg-black/60 text-sm text-gray-300"
